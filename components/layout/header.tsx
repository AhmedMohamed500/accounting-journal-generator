...
export function Header({ locale }: { locale: Locale }) {
  ...
  return <header ref={headerRef} className={`nav no-print ${landing ? "landing-nav" : ""}`} data-no-bilingual><div className="container nav-inner">
    ...
    {missionsMode ? <nav className={`nav-links missions-nav-links ${open ? "mobile-open" : ""}`}><Link href={`/${locale}`}>{ar?"الرئيسية":"Home"}</Link><Link className={pathname===`/${locale}/missions`?"active":""} href={`/${locale}/missions`}>{ar?"المهمات":"Missions"}</Link><Link href={`/${locale}/missions/daily`}>{ar?"تحدي اليوم":"Daily challenge"}</Link><Link className={pathname===`/${locale}/missions/progress`?"active":""} href={`/${locale}/missions/progress`}>{ar?"تقدمي":"My progress"}</Link><Link className="keep btn" href={`/${other}${pathname.slice(3)}`}><Languages size={17}/>{other.toUpperCase()}</Link><button className="btn icon-btn" aria-label={ar?"تغيير المظهر":"Toggle theme"} onClick={()=>setTheme(resolvedTheme==="dark"?"light":"dark")}><Moon size={17}/></button></nav>
