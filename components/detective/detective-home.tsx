...
import { ArrowLeft, ArrowRight, BadgeCheck, Clock3, FileSearch, FolderOpen, Gauge, Search, ShieldCheck, Sparkles, Trophy } from "lucide-react";
...
export function DetectiveHome({ locale }: { locale: Locale }) {
  ...
  return <div className="detective-home">
    <section className="detective-hero"><div className="detective-container detective-hero-grid"><div>
      ...
      <Link className="detective-primary" href={`/${locale}/detective/${detectiveCases.find((item) => !progress.records[item.id]?.solved)?.slug || detectiveCases[0].slug}`}><FolderOpen size={18}/>{ar ? "افتح أول ملف" : "Open your first file"}{ar ? <ArrowLeft/> : <ArrowRight/>}</Link>
