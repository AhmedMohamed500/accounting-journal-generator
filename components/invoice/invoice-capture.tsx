...
const empty: ExtractedInvoice = { supplier: "", invoiceNumber: "", taxNumber: "", date: "", dueDate: "", currency: "EGP", lines: [], subtotal: 0, discount: 0, net: 0, vatRate: 0, vat: 0, total: 0, confidence: 0, rawText: "", warnings: [] };
...
export function InvoiceCapture({ locale }: { locale: Locale }) {
  ...
  const updateLine = (id: string, key: keyof ExtractedInvoiceLine, value: string) => {
    ...
    setData((current) => ({ ...current, lines: nextLines, ...totals(nextLines), vatRate: nextLines.length ? roundCurrency(nextLines.reduce((sum, line) => sum + line.vatRate, 0) / nextLines.length) : current.vatRate }));
  };
  const addLine = () => setData((current) => ({ ...current, lines: [...current.lines, emptyLine(current.lines.length + 1)] }));
  const removeLine = (id: string) => { const lines = data.lines.filter((line) => line.id !== id); setData((current) => ({ ...current, lines, ...totals(lines) })); };
  const mismatch = data.total > 0 && Math.abs(data.total - data.net - data.vat) > .02;
  ...
  const suggest = () => {
    ...
    if (mismatch) { setStatus(ar ? "الصافي والضريبة لا يساويان الإجمالي؛ صحح المجاميع قبل إنشاء القيد." : "Net plus VAT does not equal total."); return; }
  ...
  const postFullCycle = () => {
    ...
    try {
      ...
      const document: BusinessDocument = { id: crypto.randomUUID(), number: data.invoiceNumber || `PI-${Date.now().toString().slice(-6)}`, type: "purchase-invoice", status: "posted", date: data.date, dueDate: data.dueDate || undefined, partyId: supplier.id, currency: data.currency, lines: documentLines, subtotal: data.subtotal || data.net + data.discount, discountTotal: data.discount, netTotal: data.net, vatTotal: data.vat, grandTotal: data.total, reference: data.invoiceNumber || undefined, notes: ar ? "مقروءة ومُرحّلة من مركز قراءة الفواتير" : "Captured and posted from invoice capture", linkedEntryId: postedEntry.id, createdAt: new Date().toISOString() };
  ...
  return <div className="grid gap-6">
    ...
    {data.rawText && <>
      ...
      <section className="card"><h2>{ar ? "3 — المجاميع المستخرجة" : "3 — Extracted totals"}</h2>{mismatch && <p className="warning error"><TriangleAlert size={17}/>{ar ? "الصافي + الضريبة لا يساوي الإجمالي. الترحيل متوقف حتى التصحيح." : "Net plus VAT does not equal total. Posting is blocked."}</p>}<div className="grid gap-4 md:grid-cols-2 xl:grid-cols-5"><NumberField label={ar ? "قبل الخصم" : "Subtotal"} value={data.subtotal} onChange={(value) => setField("subtotal", value)}/><NumberField label={ar ? "الخصم" : "Discount"} value={data.discount} onChange={(value) => setField("discount", value)}/><NumberField label={ar ? "الصافي" : "Net"} value={data.net} onChange={(value) => setField("net", value)}/><NumberField label={ar ? "قيمة الضريبة" : "VAT amount"} value={data.vat} onChange={(value) => setField("vat", value)}/><NumberField label={ar ? "الإجمالي" : "Total"} value={data.total} onChange={(value) => setField("total", value)}/></div></section>
