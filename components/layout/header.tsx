...
export function Header({ locale }: { locale: Locale }) {
  ...
  return <header ref={headerRef} className={`nav no-print ${landing ? "landing-nav" : ""}`} data-no-bilingual><div className="container nav-inner">
    ...
    {learningMode ? <nav className={`nav-links missions-nav-links ${open ? "mobile-open" : ""}`}><Link href={`/${locale}`}>{ar?"الرئيسية":"Home"}</Link><Link href={`/${locale}/arena`}>{ar?"Arena":"Arena"}</Link><Link className={pathname.startsWith(`/${locale}/missions`)?"active":""} href={`/${locale}/missions`}>{ar?"المهمات":"Missions"}</Link><Link className={pathname.startsWith(`/${locale}/money-flow`)?"active":""} href={`/${locale}/money-flow`}>{ar?"تدفق الأموال":"Money Flow"}</Link><Link className={pathname.includes("detective")?"active":""} href={`/${locale}/detective`}>{ar?"المحقق":"Detective"}</Link><Link className="keep btn" href={`/${other}${pathname.slice(3)}`}><Languages size={17}/>{other.toUpperCase()}</Link><button className="btn icon-btn" aria-label={ar?"تغيير المظهر":"Toggle theme"} onClick={()=>setTheme(resolvedTheme==="dark"?"light":"dark")}><Moon size={17}/></button></nav>
