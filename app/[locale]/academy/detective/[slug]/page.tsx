...
export function generateStaticParams() {
  return ["ar", "en"].flatMap((locale) => detectiveCases.map((item) => ({ locale, slug: item.slug })));
}

export default async function Page({ params }: { params: Promise<{ locale: Locale; slug: string }> }) {
  const { locale, slug } = await params;
  const caseDefinition = getDetectiveCase(slug);
  if (!caseDefinition) notFound();
  return <DetectiveWorkspace caseDefinition={caseDefinition} locale={locale}/>;
}
