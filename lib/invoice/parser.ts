...
export function parseInvoiceText(raw:string):ExtractedInvoice{
  ...
  let total=amount(text,["grand total","total amount","invoice total","amount due","total due","الإجمالي شامل الضريبة","الإجمالي بعد الضريبة","إجمالي الفاتورة","إجمالي المستحق","المبلغ المستحق","إجمالي المبلغ","الإجمالي"]),vat=amount(text,["VAT amount","tax amount","total VAT","VAT total","قيمة ضريبة القيمة المضافة","إجمالي ضريبة القيمة المضافة","ضريبة القيمة المضافة","إجمالي الضريبة","قيمة الضريبة","الضريبة"]),discount=amount(text,["discount total","total discount","إجمالي الخصم","الخصم"]),subtotal=amount(text,["subtotal","gross amount","total before discount","الإجمالي قبل الخصم","الإجمالي الفرعي","إجمالي المبيعات","المجموع"]),net=amount(text,["net amount","amount before tax","taxable amount","net total","الصافي","الإجمالي قبل الضريبة","المبلغ قبل الضريبة","الخاضع للضريبة","إجمالي المبيعات","قبل الضريبة"]);
  ...
  const rateMatch=text.match(/(?:VAT|ضريبة القيمة المضافة|الضريبة)\s*(?:rate|نسبة)?\s*[:#-]?\s*(\d+(?:\.\d+)?)\s*%/i);let vatRate=rateMatch?Number(rateMatch[1]):net&&vat?roundCurrency(vat/net*100):0;
  ...
  if(!identity){warnings.push("possibly-not-invoice");lines=[];subtotal=0;discount=0;net=0;vatRate=0;vat=0;total=0;}
