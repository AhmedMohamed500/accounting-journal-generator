...
function contextualDescription(rows:string[],index:number,ignore:RegExp){
  const tableHeader=/(اسم الكود|كود الصنف|الوصف|سعر الوحدة|قيمة المبيعات|نوع الوحدة|الكمية|item code|description|unit price|quantity)/i;
  return[index-1,index+1].map((position,neighborIndex)=>{
    const source=rows[position]||"";if(!source||ignore.test(source)||tableHeader.test(source))return"";
    let description=source.replace(/\b(?:EG-[A-Z0-9-]{4,}|C\d{2,}|[A-Z0-9]{18,})\b/gi," ").replace(/توميرب/g,"بريموت").replace(/ةصوب/g,"بوصة").replace(/بوصة\s*(\d+)/g,"$1 بوصة").replace(/\s+/g," ").trim();
    if(neighborIndex===0)description=description.replace(/\s+\d+(?:[,.]\d+)?\s*\/?\s*$/,"").trim();
    return description.replace(/^[/|:;.,\s-]+|[/|:;.,\s-]+$/g,"").trim();
  }).filter(description=>description.replace(/[^\p{L}]/gu,"").length>=3).join(" ").slice(0,140);
}
...
function lineItems(text:string,fallbackRate:number):ExtractedInvoiceLine[]{
  ...
  const rows=linesOf(text);
  for(let index=0;index<rows.length;index++){const source=rows[index];if(ignore.test(source))continue;const hasEtaCode=/\b(?:EG-[A-Z0-9-]{4,}|C\d{2,}|[A-Z0-9]{18,})\b/i.test(source),numericSource=source.replace(/\b(?:EG-[A-Z0-9-]{4,}|C\d{2,}|[A-Z0-9]{18,})\b/gi," ");let tokens=(numericSource.match(amountToken)||[]).map(token=>token.trim()).filter(Boolean);if(hasEtaCode&&tokens.length===2){const previousTokens=(rows[index-1]?.match(amountToken)||[]).filter(token=>!token.endsWith("%")),quantity=previousTokens.at(-1);if(quantity&&numeric(quantity)>0&&numeric(quantity)<=1_000_000)tokens=[quantity,...tokens];}if(tokens.length<3||tokens.some(token=>/^20\d{2}$/.test(token)))continue;const inlineDescription=clean(numericSource.replace(amountToken," ").replace(/\s+/g," ").replace(/^[.)\-\d\s]+/,"")),description=(hasEtaCode?contextualDescription(rows,index,ignore):"")||inlineDescription;if(description.length<2||description.length>140)continue;const reversed=[...tokens].reverse(),candidates=[lineCandidate(tokens,0,fallbackRate),lineCandidate(tokens,1,fallbackRate),lineCandidate(reversed,0,fallbackRate),lineCandidate(reversed,1,fallbackRate)].filter(Boolean) as NonNullable<ReturnType<typeof lineCandidate>>[];if(!candidates.length)continue;const candidate=candidates.sort((a,b)=>a.score-b.score)[0],key=`${description.replace(/\s+/g," ").toLocaleLowerCase("ar-EG")}|${candidate.quantity}|${candidate.unitPrice}|${candidate.total}`;if(seen.has(key))continue;seen.add(key);results.push({id:`line-${results.length+1}`,description,quantity:candidate.quantity,unitPrice:candidate.unitPrice,discount:candidate.discount,vatRate:candidate.vatRate,net:candidate.net,vat:candidate.vat,total:candidate.total});if(results.length>=200)break;
  }
  return results;
}
